...
export default [
  http.get(
    `https://${env("API_HOST", "api.example.com")}/users/${get("user_id")}`,
  ),
];
```
...
http.post("https://api.example.com/orders").before(
...
export interface HttpRequest {
  ...
  // Updates outgoing body and options (headers).
  // Objects are automatically serialized to JSON.
  set(
    body: object | string | ArrayBuffer | Uint8Array | null,
...
```typescript
http.post("https://api.example.com/auth/login").after({
  expect: {
    status: 200,
...
http.get("https://api.example.com/account/profile").after((res, req) => {
