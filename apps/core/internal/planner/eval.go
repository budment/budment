...
func (e *Evaluator) injectMockDSL(vm *goja.Runtime) error {
	...
	vm.Set("console", map[string]interface{}{
		"log":   func(msg string) any { return createMockNode("log", map[string]any{"message": msg}) },
		"warn":  func(msg string) any { return createMockNode("log", map[string]any{"message": "[WARN] " + msg}) },
		"error": func(msg string) any { return createMockNode("log", map[string]any{"message": "[ERROR] " + msg}) },
	})
	vm.Set("tag", func(key string, value string) any {
		return createMockNode("tag", map[string]any{"key": key, "value": value})
	})
	vm.Set("set", func(key string, val goja.Value) any {
		var strVal string

		// Force string coercion to trigger the Proxy's toString() trap
		strRepresentation := val.String()

		if strings.HasPrefix(strRepresentation, "{{") && strings.HasSuffix(strRepresentation, "}}") {
			// Case 1: Proxy token (e.g., {{obj}{a}}) generated by get()
			strVal = strRepresentation
		} else if obj, ok := val.(*goja.Object); ok && obj.ClassName() != "Function" {
			// Case 2: Valid JS Object/Array -> Serialize to JSON string
			b, _ := json.Marshal(obj.Export())
			strVal = string(b)
		} else {
			// Case 3: Primitive values (string, number, boolean)
			strVal = strRepresentation
		}
